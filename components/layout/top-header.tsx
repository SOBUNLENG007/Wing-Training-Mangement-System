...
const user = useAuthStore((state) => state.user)
  ...
  // 2. SAFE INITIALS LOGIC
  // We use optional chaining ?. and filter(Boolean) to avoid errors
  const initials = user?.name
    ? user.name
        .split(" ")
        .filter(Boolean)
  ...
  // 4. SAFE NOTIFICATION LOGIC
  const unreadCount = mockNotifications ? mockNotifications.filter((n) => !n.read).length : 0;

  // If you don't want the header to show at all until the user is loaded:
  // if (!user) return <header className="h-[74px] border-b bg-white" />;
